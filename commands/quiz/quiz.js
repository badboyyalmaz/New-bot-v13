...
module.exports = {
	name: 'quiz',
	description: 'Juega un quiz',
	aliases: ['quizs', 'questionnaire', 'cuestionario'],
	cooldown: 10,
	cat: 'games',
	botpermissions: ['SEND_MESSAGES', 'EMBED_LINKS'],
	async execute(message, args, client) {
		const lang = await message.translate('QUIZ');
		const verify = await guild.findOne({
			...
			if (find) {
				...
				message.channel
					...
					.then(() => {
						...
						message.channel
							.awaitMessages({ filtro, max: 1, time: 30000, errors: ['time'] })
							.then(async collected => {
								if (collected.first().author.bot) return;
								if (
									quiz.reponse.toLowerCase() ===
									collected.first().content.toLowerCase()
								) {
									if (find.xp + 10 == 100) {
										let update = await QuizBD.findOneAndUpdate(
											{ userID: message.author.id },
											{
												$set: {
													...
													level: find.level + 1,
													quizs: find.quizs + 1,
													Corrects: find.Corrects + 1
												}
											},
											{ new: true }
										);
										return message.succesMessage(
											lang.okXP
												.replace('{user}', collected.first().author.username)
												.replace('level', find.level + 1)
										);
									}
									let update = await QuizBD.findOneAndUpdate(
										{ userID: message.author.id },
									...
									return message.succesMessage(
										lang.ok.replace('{user}', collected.first().author.username)
									);
								} else {
									let update = await QuizBD.findOneAndUpdate(
										{ userID: message.author.id },
										{ $set: { quizs: find.quizs + 1 } },
										{ new: true }
									);
